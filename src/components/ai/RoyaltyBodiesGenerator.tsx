...
const THINKING_MESSAGES = [
  'Researching your country\u2019s music bodies…',
  'Checking writer & performer rights…',
  'Prioritising essential registrations…',
    ...
    <div className="space-y-8">
      ...
      {result && (
        <AIResultPanel title={`Royalty Bodies for ${result.country}`}>
          <div className="space-y-8">
            <div>
              <p className="text-zinc-300 text-sm leading-relaxed min-h-[1.5em]">
                <TypewriterText text={result.summary} speed={14} charsPerTick={2} onComplete={() => setSummaryDone(true)} />
              </p>
              <p className={cn('text-xs text-purple-400 font-semibold mt-2 transition-opacity duration-500', summaryDone ? 'opacity-100' : 'opacity-0')}>
                Estimated timeline: {result.estimatedTimeline}
              </p>
            </div>

            <div className={cn('space-y-8 transition-opacity duration-500', summaryDone ? 'opacity-100' : 'opacity-0')}>
              <div className="space-y-3">
                {result.organizations.map((org, i) => (
                  <div key={i} className="bg-[#080814] border border-purple-500/20 rounded-xl overflow-hidden hover:border-pink-500/30 transition-colors">
                    <div className="px-5 py-3 border-b border-purple-500/10 flex items-center justify-between flex-wrap gap-2">
